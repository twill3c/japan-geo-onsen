...
const m = await page.evaluate(()=>{
  ...
  const t=document.querySelector('.feature-panel .surroundings > table');
  t.scrollIntoView({block:'start'});
  const pr=panel.getBoundingClientRect(), tr=t.getBoundingClientRect();
  // パネルの中を上端までスクロールしたとき、表の先頭行が枠の中に見えるか
  const firstRow=t.querySelector('thead tr').getBoundingClientRect();
  const hit=document.elementFromPoint(firstRow.left+firstRow.width/2, firstRow.top+firstRow.height/2);
  return {
    panelTop:Math.round(pr.top), panelBottom:Math.round(pr.bottom),
    tableTop:Math.round(tr.top), headRowTop:Math.round(firstRow.top), headRowBottom:Math.round(firstRow.bottom),
    headInsidePanel: firstRow.top>=pr.top-0.5 && firstRow.bottom<=pr.bottom+0.5,
    topmostAtHeadRow: hit ? (hit.closest('.surroundings') ? 'surroundings の表' : hit.className || hit.tagName) : null,
